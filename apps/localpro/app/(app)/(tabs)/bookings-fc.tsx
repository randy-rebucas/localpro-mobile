...
type FilterStatus = 'all' | FacilityBooking['status'];

export default function BookingsFCScreen() {
  ...
  const handleViewDetails = (bookingId: string) => {
    router.push(`/(stack)/facility-booking/${bookingId}` as any);
  };

  const handleCancel = (bookingId: string) => {
    Alert.alert(
      'Cancel Contract',
      `Are you sure you want to cancel this facility care contract?`,
      [
        { text: 'No', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              // TODO: Implement API call to cancel facility booking
              // await FacilityCareService.cancelBooking(bookingId);
              Alert.alert('Success', 'Contract cancelled successfully.');
            } catch {
              Alert.alert('Error', 'Failed to cancel contract. Please try again.');
            }
          },
        },
      ]
  ...
  const renderBookingCard = ({ item }: { item: FacilityBooking }) => {
      ...
      >
        <View style={styles.bookingHeader}>
          <View style={styles.bookingInfo}>
            <Text style={styles.bookingTitle}>Facility Care Contract</Text>
          ...
          <View style={styles.header}>
            ...
            <View style={styles.headerContent}>
              <View>
                ...
                <Text style={styles.subtitle}>View and manage your facility care contracts</Text>
              </View>
            </View>
            ...
            <View style={styles.emptyContainer}>
              <EmptyState
                ...
                title={
                  activeFilter === 'all'
                    ? 'No Contracts Yet'
                    : `No ${filters.find((f) => f.key === activeFilter)?.label} Contracts`
                }
                subtitle={
                  activeFilter === 'all'
                    ? 'Your facility care contracts will appear here when you make a booking'
                    : `You don't have any ${filters.find((f) => f.key === activeFilter)?.label.toLowerCase()} contracts at the moment`
                }
              />
              {activeFilter === 'all' && (
                <TouchableOpacity
                  style={[styles.exploreButton, { backgroundColor: colors.primary[600] }]}
                  onPress={() => router.push('/(app)/(tabs)/services-fc' as any)}
                  activeOpacity={Platform.select({ ios: 0.7, android: 0.8 })}
                >
                  <Text style={styles.exploreButtonText}>Browse Services</Text>
                </TouchableOpacity>
              )}
            </View>
          )
